...
def run_experiment_multi_seed(
    ...
    best_loss = float('inf')
    best_seed = None
    # Separately track the best ACCEPTABLE seed (one that meets all thresholds).
    # If thresholds are set, the caller wants min-loss-among-acceptable, not
    # first-acceptable. (Old code overwrote best_result with the first seed
    # that met thresholds, even if its loss was worse than a prior seed.)
    best_acceptable_result = None
    best_acceptable_loss = float('inf')
    best_acceptable_seed = None
    all_final_losses = []
    any_threshold_set = (loss_threshold is not None or min_norm_threshold is not None
                        or max_norm_threshold is not None or min_angle_threshold is not None)
    ...
    for seed in iterator:
        ...
        # Check whether this seed meets ALL thresholds (geometry + loss).
        # If so, update the "best acceptable" tracker — but ONLY if it's the
        # lowest-loss acceptable so far (NOT overwriting a better one).
        meets_all = True
        met_reasons = []
        if loss_threshold is not None:
            if final_loss < loss_threshold:
                met_reasons.append(f"loss={final_loss:.6f}")
            else:
                meets_all = False
        if min_norm_threshold is not None:
            if geom['min_norm'] >= min_norm_threshold:
                met_reasons.append(f"min_norm={geom['min_norm']:.3f}")
            else:
                meets_all = False
        if max_norm_threshold is not None:
            if geom['max_norm'] <= max_norm_threshold:
                met_reasons.append(f"max_norm={geom['max_norm']:.3f}")
            else:
                meets_all = False
        if min_angle_threshold is not None:
            if geom['min_angle'] >= min_angle_threshold:
                met_reasons.append(f"min_angle={geom['min_angle']:.1f}°")
            else:
                meets_all = False

        if any_threshold_set and meets_all and final_loss < best_acceptable_loss:
            best_acceptable_result = result
            best_acceptable_loss = final_loss
            best_acceptable_seed = seed
            if verbose:
                print(f"\nNew best acceptable seed {seed}: {', '.join(met_reasons)}")

        # Early stop: if user set thresholds and this seed meets them, we can
        # stop searching. We RETURN the lowest-loss acceptable found so far
        # (which is either this seed, or an earlier-acceptable seed with lower
        # loss — we keep whichever).
        if any_threshold_set and meets_all:
            break

    # Return preference: best-among-acceptable if any; else best-loss overall.
    if best_acceptable_result is not None:
        best_result = best_acceptable_result
        best_loss = best_acceptable_loss
        best_seed = best_acceptable_seed
